...
const rooms: Record<string, Room> = {
  "spiral-stair": {
    ...
    name: "Spiral Stair",
    description:
      "A narrow iron staircase spirals upward through the stone tower, disappearing into shadow. Cool salt air drifts down from above, carrying the faint smell of oil and rust.",
  ...
  "lamp-room": {
    ...
    name: "Lamp Room",
    description:
      "Glass panels curve around a great brass lamp, its lens catching the last grey light of dusk. Far below, whitecaps break against the rocks in silence.",
    mapId: "map-lamp-room",
  ...
  "keepers-kitchen": {
    ...
    description:
      "A blackened kettle sits cold on the stove beside a stack of unopened letters. Dust covers the table, as if the keeper left in a hurry.",
  ...
  rocks: {
    ...
    name: "Rocks",
    description:
      "Waves hiss over dark, barnacled rocks at the base of the lighthouse. The tower looms overhead, its light not yet lit.",
    mapId: "map-rocks",
...
const mapRoomEls = document.querySelectorAll<HTMLElement>(".map-room");
const illustrationEls = document.querySelectorAll<HTMLElement>(".room-illustration");
...
function render(): void {
  ...
  const exitLabels = (Object.keys(room.exits) as Direction[]).map(
...
function move(direction: Direction): void {
  ...
  messageEl.textContent =
    room.blocked[direction] ?? "You can't go that way.";
}
